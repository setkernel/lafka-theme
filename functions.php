					...
					<div id="comment-<?php comment_ID(); ?>" class="comment-body">
						...
						comment_reply_link(
							array_merge(
								$args,
								array(
									'reply_text' => esc_html__( 'Reply', 'lafka' ),
									'depth'      => $depth,
...
if ( ! function_exists( 'lafka_comments_valid_for_post' ) ) {
	/**
	 * Check if all comments are generated by the ActionScheduler
	 *
	 * Avoids loading every comment row for the post (the previous implementation
	 * called `get_comments()` with no `number` cap, which on posts with hundreds
	 * of ActionScheduler-authored rows fanned into a full SELECT * on every
	 * single-post page-load). Two narrow `COUNT(*)` queries replace it, plus a
	 * static per-request memo so `comments.php` and `comments_template` calls
	 * for the same post hit memory after the first read.
	 *
	 * @param int $post_id
	 *
	 * @return array Assoc array: ['valid_comments' => bool, 'valid_comments_count' => int]
	 */
	function lafka_comments_valid_for_post( $post_id ) {
		...
		$cache[ $post_id ] = array(
			'valid_comments'       => $total > 0 && $total !== $as_count,
			'valid_comments_count' => max( 0, $total - $as_count ),
		);
