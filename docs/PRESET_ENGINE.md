...
- **theme_mod-default layer (TML)** — the mechanism by which a preset supplies the
  *default* value for the 55 keys in `LAFKA_PRESET_CHROME_WHITELIST` (`lafka_accent_color`,
...
lafka-theme/
  ...
  incl/presets/
    ...
    lafka-preset-tokens.php           # LAFKA_PRESET_TOKEN_WHITELIST (88) + LAFKA_PRESET_CHROME_WHITELIST (55) + LAFKA_PRESET_CRITICAL_KEYS (pure-data constants)
...
```jsonc
{
  ...
  "chrome": {                      // TML defaults; keys MUST be in LAFKA_PRESET_CHROME_WHITELIST
    ...
    // (lafka_legacy_migrate_map() destinations) dynamic-css emits — 55 keys
  },
...
## 5. theme_mod-default layer (50 call sites in dynamic-css.php)

Every chrome read in `dynamic-css.php` is wrapped — 50 `lafka_preset_default()` call sites
covering all 55 whitelisted keys (the six `lafka_h1_font`…`lafka_h6_font` arrays share one
